...
Before using advanced deployment options, ensure you have completed the [Simple Solo Setup](../simple-setup/) and have:
...
After deployment, you can validate that mirror ingestion is working by creating a few transactions:
...
If you have already deployed Explorer (next step), you can inspect those transactions at `http://localhost:8080`.

If you need direct access to mirror services for debugging or integration tests, use port-forwarding:
...
For SDK-based transaction and topic examples against this network, see [Using Solo with Hiero JavaScript SDK](../using-solo/javascript-sdk.md).
