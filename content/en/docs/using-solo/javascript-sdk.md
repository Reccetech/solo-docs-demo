...
## Using Other Hiero SDKs (Java, Go, Rust, C++)

The same Solo network can be used by all official Hiero SDKs. The main differences are project setup and language-specific APIs.

Use these common local endpoints with your SDK client:

| Service | Local Endpoint | Notes |
|---|---|---|
| Consensus gRPC | `127.0.0.1:50211` | Primary transaction endpoint |
| Mirror gRPC | `127.0.0.1:5600` | Mirror query endpoint (if enabled) |
| Mirror REST | `http://127.0.0.1:5551/api/v1` | Useful for REST-based verification |
| JSON-RPC Relay | `http://127.0.0.1:7546` | For EVM-compatible workflows |

### Java SDK

1. Clone the Java SDK repository and follow its build/setup instructions.
2. Configure operator credentials from Solo (`accountId` and `privateKey`) in your app config.
3. Point your client to the local Solo network (`127.0.0.1:50211`).
4. Run a simple transaction flow (for example: create account or create topic) and verify in Explorer.
...
1. Clone or create a Go project that uses the Hiero Go SDK.
2. Configure operator credentials generated by Solo.
3. Connect to the Solo node endpoint (`127.0.0.1:50211`).
4. Execute a basic transaction and confirm results in Mirror REST or Explorer.
...
1. Create a Rust project and add the Hiero Rust SDK dependency.
2. Load Solo-generated operator account credentials.
3. Configure the client for local Solo node access (`127.0.0.1:50211`).
4. Submit and query a simple transaction to validate connectivity.
...
1. Clone the C++ SDK and complete build prerequisites.
2. Configure local Solo operator credentials in your test program.
3. Connect to the local node endpoint (`127.0.0.1:50211`).
4. Run a basic operation and verify output through Mirror/Explorer.

Official repository:
- <https://github.com/hiero-ledger/hiero-sdk-cpp>

### Cross-SDK Verification Checklist

Regardless of SDK language, validate these items:

- Node endpoint reachable (`127.0.0.1:50211`)
- Transaction submit succeeds
- Transaction appears in Mirror REST (`/api/v1/transactions`)
- Account/topic state matches expected values
- Explorer shows the same activity
